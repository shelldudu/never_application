...
namespace B2C.Message.SqlData.Repository.Xml
{
    /// <summary>
    /// sqlserver
    /// </summary>
    /// <seealso cref="Never.EasySql.EmbeddedDaoBuilder" />
    public class SqlServiceRepositoryDaoBuilder : EmbeddedDaoBuilder
    {
        private readonly Func<string> connstring = null;

        public SqlServiceRepositoryDaoBuilder(Func<string> connstring)
        {
        ...
        public override string[] EmbeddedSqlMaps
        {
            get
            {
                var maps = base.GetXmlContentFromAssembly(System.Reflection.Assembly.GetExecutingAssembly(), (x, f) =>
                {
                    ...
                    if (id.IsEquals("sqlserver.repos", StringComparison.CurrentCultureIgnoreCase))
                        return true;
