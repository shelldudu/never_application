...
namespace B2C.Calc.Banks.Repayments
{
    ...
    /// 按期付息N期还本，利息一直以原金额计算
    /// </summary>
    public class 按期付息N期还本 : RepaymentCalc, IRepaymentCalc
    {
        #region field
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="按期付息N期还本"/> class.
        /// </summary>
        /// <param name="dateType">结算类型</param>
        /// <param name="startDate">开始结算日期</param>
        /// <param name="principalPeriods">本金的还款信息</param>
        public 按期付息N期还本(RepaymentTimeUnitType dateType, DateTime startDate, IEnumerable<PrincipalRepayType> principalPeriods)
        ...
        public override IEnumerable<RepaymentModel> GetRepayments(decimal principal, RepaymentRateType rateType, decimal rate, decimal deadline, int periods = 1, bool autoFixPrincipalToInteger = true)
        {
            ...
            if (!autoFixPrincipalToInteger)
            {
                ...
                for (var i = 1; i <= periods; i++)
                {
                    ...
                    {
                        list.Add(new RepaymentModel() { Principal = prinList[j], Interest = this.Strategy.GetNUnitAmount(principal * rate * deadline, rateType) / periods, Period = i, PayDate = this.Strategy.GetPayTime(this.StartDate, i * deadline / periods) });
                        j++;
                    }
                    else
                    {
            ...
            for (var i = 1; i <= periods; i++)
            {
                if (this.principalPeriods.ElementAt(i - 1).PayPrincipal)
                {
                    list.Add(new RepaymentModel() { Principal = prinList[k], Interest = this.Strategy.GetNUnitAmount(principal * rate * deadline, rateType) / periods, Period = i, PayDate = this.Strategy.GetPayTime(this.StartDate, i * deadline / periods) });
                    k++;
                }
                else
                {
